...
```{r setup, include=FALSE}
  ...
  colscale = scale_color_manual(
    ...
    "#2BE5F6"
      ...
      might_synonyms = c("might", "possible", "could")
    probably_synonyms = c("probably", "likely")
...
load_data_for_plotting = function(fname) {
  ...
  d = d %>% 
    ...
    mutate(modal_normalized = recode_factor(modal, possible = "might", could="might", likely="probably", .ordered = TRUE)) 
  return(d)
}

 plot_posterior = function(modal1, modal2, exp_data, hdi_data) {

     orig_cond_name = paste(modal1, modal2, sep="-")

   
     if (modal1 %in% might_synonyms) {
       modal1 = "might"
     }
     if (modal2 %in% might_synonyms) {
       modal2 = "might"
     }
     if (modal1 %in% probably_synonyms) {
       modal1 = "probably"
     }
      if (modal2 %in% probably_synonyms) {
       modal2 = "probably"
     }

     cond_name = paste(modal1, modal2, sep="-")

     if (cond_name %in% c("probably-might", "might-bare", "might-bare_not", "probably-bare", "probably-bare_not")) {
          cond_name = paste(modal2, modal1, sep="-")
     ...
     pred_data = hdi_data %>% 
       ...
       mutate(modal = factor(modal, levels = all_modals, labels = modals_labels, ordered = TRUE)) %>%
       mutate(modal_normalized = modal)
      

      exp_data = exp_data %>% 
        group_by(percentage_blue, modal, modal_normalized) %>%
        summarise(rating_m = mean(rating), ci_low = ci.low(rating), ci_high = ci.high(rating)) %>%
        mutate(type ="exp")
        
      merged_data = merge(pred_data, exp_data, by=c("percentage_blue", "modal_normalized"))

      model = lm(rating_pred_m ~ rating_m, data=merged_data)
      cat(paste("R^2:", orig_cond_name, summary(model)$r.squared), "\n")
...
```{r call, echo=FALSE}
  ...
  model_params_all = read.csv(file = "./runs/classic-semantics/mle_params.csv")
...
```{r fig1, fig.width=10, fig.height=10.625, echo=FALSE, warning=FALSE}
  ...
  hdi_data = read.csv("./runs/classic-semantics/hdi_samples.csv")
  ...
  get_grid_plot = function(cond) {
    ...
    if (modal1 == "think" || 
        ...
        modal1 == "looks_like" || 
        modal2 == "looks_like" ||
        all(c(modal1, modal2) %in% might_synonyms) ||
        all(c(modal1, modal2) %in% probably_synonyms)) {
      return (ggplot())
    }
    if (cond < 21) {
  ...
  g1 = do.call("arrangeGrob", c(plots[c(1:3, 6,10,16:19, 22, 24,26)], ncol=3, left="utterance rating", bottom="event probability"))
  grid.arrange(g1, btm_legend, heights=c(32, 2))
```

```{r fig.width=10, fig.height=5.125, echo=FALSE}
  g2 = do.call("arrangeGrob", c(plots[c(27, 29:32)], ncol=3, left="utterance rating", bottom="event probability"))
