...
reparam = function(d, modals=NULL) {
  if (is.null(modals)) {
    modals = c("bare", "could",  "might", "probably", "possible", "likely", "bare_not")
  }
...
setwd("/Users/sebschu/Dropbox/Uni/RA/adaptation/possible-might-project/models/")
...
d = convertData("./1_threshold_modals/runs/classic-semantics/run1_output.json")
d = reparam(d, modals = c("might", "bare", "probably", "bare_not"))
c1 = tidy_sampleList(d, 2, 1, list())

d = convertData("./1_threshold_modals/runs/classic-semantics/run2_output.json")
d = reparam(d, modals = c("might", "bare", "probably", "bare_not"))
c2 = tidy_sampleList(d, 2, 2, list())
...
write.csv(x = rbind(final_params_mu, final_params_low, final_params_high),
          file = "./1_threshold_modals/runs/classic-semantics/mle_params.csv")



### no-near synonyms

d = convertData("./1_threshold_modals/runs/no-near-synonyms/run1_output.json")
d = reparam(d)
c1 = tidy_sampleList(d, 2, 1, list())

d = convertData("./1_threshold_modals/runs/no-near-synonyms/run2_output.json")
d = reparam(d)
c2 = tidy_sampleList(d, 2, 2, list())
...
write.csv(x = rbind(final_params_mu, final_params_low, final_params_high),
          file = "./1_threshold_modals/runs/no-near-synonyms/mle_params.csv")
