...
class ThresholdModel:
  ...
  def compute_trans_probs(self, src_theta_alphas, src_theta_betas, src_costs,
      ...
      for i, utt in enumerate(self.config["utterances"]):
        if utt["has_theta"] and "copy_theta" not in utt:
  ...
  def init_trace(self):
      ...
      for i, utt in enumerate(self.config["utterances"]):
          ...
          if utt["has_theta"] and "copy_theta" in utt:
             theta_alphas[i] = theta_alphas[self.expressions2idx[utt["copy_theta"]]]
             theta_betas[i] = theta_betas[self.expressions2idx[utt["copy_theta"]]]
